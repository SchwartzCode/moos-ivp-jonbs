...
bool HazardMgrX::Iterate()
{
  ...
  if (m_first_iterate)
  {
    if (m_host_community == "jake")   //yo this is some gross coding
      m_other_community = "kasper";
    else if (m_host_community == "kasper")
      m_other_community = "jake";
    m_first_iterate = false;
  }

  if(!m_sensor_config_requested && m_sensor_config_change)
  {
    postSensorConfigRequest();
    m_sensor_config_change = false;
  }
...
bool HazardMgrX::OnStartUp()
{
  ...
  m_final_hazard_set.setSource(m_host_community);
  m_final_hazard_set.setName(m_report_name);
  m_final_hazard_set.setRegion(m_search_region);
...
void HazardMgrX::registerVariables()
{
  ...
  Register("HAZARDSET_EVAL", 0);

  //Debugging
  Register("INPUT", 0);
  Register("MEDIARY", 0);
  Register("CHECK_ONE", 0);
  Register("CHECK_TWO", 0);
  Register("A", 0);
  Register("B", 0);
...
bool HazardMgrX::handleMailSensorOptionsSummary(string input)
{
  m_options_vec = parseString(input, ':');
  unsigned int i, vsize = m_options_vec.size();
  
  string a = m_options_vec[0];
  string b = biteStringX(a, ',');
  string c = biteStringX(b, '=');
  int curr_width = stoi(b);

  for(i=0; i<vsize; i++) {
    string a = m_options_vec[i];
    string b = biteStringX(a, ',');
    string c = biteStringX(b, '=');
    curr_width = stoi(b);
...
//
bool HazardMgrX::handleMailHazardReport(string input)
{

  XYHazard new_hazard = string2Hazard(input);
  
  m_final_hazard_set.addHazard(new_hazard);
  
  string summary_report = m_final_hazard_set.getSpec("final_report");
  vector<string> no_commas = parseString(input,',');
  string final_report;

  for (int i=0; i<no_commas.size(); i++)
    final_report += no_commas[i] + "|";

  string report_prefix = "src_node=" + m_host_community + ",dest_node=" + m_other_community + ",var_name=FOUND_POINTS,string_val=";
  
  Notify("NODE_MESSAGE_LOCAL", report_prefix + final_report);
...
void HazardMgrX::handleMailReportRequest()
{
  ...
  m_final_hazard_set.findMinXPath(20);
  //unsigned int count    = m_hazard_set.findMinXPath(20);
  
  
  
  string combined_report = m_final_hazard_set.getSpec("final_report") + m_other_comm_report;

  vector<string> erma = parseString(combined_report, '#');
  combined_report = "";

  for(int i=0; i<erma.size(); i++)
  {
  ...
  {
    Notify("CHECK_ONE", m_final_hazard_set.getSpec());
    Notify("HAZARDSET_REPORT", combined_report);
    m_report_sent = true;
  }
}

//----------------------------------------------------------
// Procedure: handleMailFoundPoints
//      recieves and reorganizes node messages from other vehicle

void HazardMgrX::handleMailFoundPoints(string input)
{

  vector<string> storage = parseString(input, '|');
...
void HazardMgrX::handleMailMissionParams(string str)
{
  ...
  for(i=0; i<vsize; i++) {
    ...
    if (param=="penalty_missed_hazard")
      pen_missed_haz = stoi(value);
    else if (param=="penalty_nonopt_hazard")
      pen_nonopt_haz = stoi(value);
    else if (param=="penalty_false_alarm")
      pen_false_alarm = stoi(value);
    else if (param == "penalty_max_time_over")
      pen_time_lmt = stoi(value);
    else if (param == "penalty_max_time_rate")
      pen_rate_over_time_lmt = stod(value);
    else if (param == "transit_path_width")
      m_transit_path_width = stoi(value);
    else if (param == "search_region") {
      biteStringX(value, '{');
      string points = biteStringX(value, '}');

      vector<string> ind_points = parseString(points, ':');

        for (int i=0; i<ind_points.size(); i++)
        {
          string y = ind_points[i];
          string x = biteStringX(y, ',');

          m_search_region.add_vertex(stoi(x), stoi(y));
        }
    }
